...
use crate::{Trait, common::bound::Bound, panic};

/// The parsed settings of a type-level (or variant-level) `PartialOrd` attribute.
pub(crate) struct TypeAttribute {
    pub(crate) bound: Bound,
}

#[derive(Debug)]
/// Parses `PartialOrd` metas; the `enable_*` switches describe which parameters are allowed at the current position.
...
impl TypeAttributeBuilder {
    /// Parses one `PartialOrd` meta into a `TypeAttribute`, rejecting parameters that are not enabled here.
    ...
    /// Scans the `#[educe(...)]` attributes of an item (typically an enum variant) and parses its `PartialOrd` meta if present.
