...
/// The parsed settings of a field-level `Copy` attribute.
pub(crate) struct FieldAttribute;

/// Parses field-level `Copy` metas; the `enable_*` switches describe which parameters are allowed for the current shape of data.
pub(crate) struct FieldAttributeBuilder;

impl FieldAttributeBuilder {
    /// Parses one field-level `Copy` meta into a `FieldAttribute`, rejecting parameters that are not enabled here.
    ...
    /// Scans the `#[educe(...)]` attributes of a field and parses its `Copy` meta if present.
