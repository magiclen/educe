...
#[derive(Debug)]
/// Parses `Deref` metas; the `enable_*` switches describe which parameters are allowed at the current position.
pub(crate) struct TypeAttributeBuilder {
    pub(crate) enable_flag: bool,
}

impl TypeAttributeBuilder {
    /// Parses one `Deref` meta into a `TypeAttribute`, rejecting parameters that are not enabled here.
    ...
    /// Scans the `#[educe(...)]` attributes of an item (typically an enum variant) and parses its `Deref` meta if present.
