...
/// The parsed settings of a field-level `Deref` attribute.
pub(crate) struct FieldAttribute {
    pub(crate) flag: bool,
    pub(crate) span: Span,
}

/// Parses field-level `Deref` metas; the `enable_*` switches describe which parameters are allowed for the current shape of data.
pub(crate) struct FieldAttributeBuilder {
    pub(crate) enable_flag: bool,
}

impl FieldAttributeBuilder {
    /// Parses one field-level `Deref` meta into a `FieldAttribute`, rejecting parameters that are not enabled here.
    ...
    /// Scans the `#[educe(...)]` attributes of a field and parses its `Deref` meta if present.
