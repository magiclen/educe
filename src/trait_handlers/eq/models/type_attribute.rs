...
impl TypeAttributeBound {
    pub fn into_punctuated_where_predicates_by_generic_parameters(self, params: &Punctuated<GenericParam, Comma>) -> Punctuated<WherePredicate, Comma> {
        match self {
            ...
            TypeAttributeBound::Auto => create_where_predicates_from_generic_parameters(params, &syn::parse(quote!(core::cmp::Eq).into()).unwrap()),
...
impl TypeAttributeBuilder {
    pub fn from_eq_meta(&self, meta: &Meta) -> TypeAttribute {
        ...
        let correct_usage_for_bound = {
            let usage = vec![stringify!(#[educe(Eq(bound))]), stringify!(#[educe(Eq(bound = "where_predicates"))]), stringify!(#[educe(Eq(bound("where_predicates")))])];
            ...
            Meta::List(list) => {
                ...
                for p in list.nested.iter() {
                    match p {
                        ...
                        _ => panic::attribute_incorrect_format("Eq", &correct_usage_for_eq_attribute)
                    }
                }
            }
            Meta::NameValue(_) => panic::attribute_incorrect_format("Eq", &correct_usage_for_eq_attribute),
            Meta::Word(_) => ()
        }
