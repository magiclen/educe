...
/// The parsed settings of a type-level (or variant-level) `Clone` attribute.
pub(crate) struct TypeAttribute {
    pub(crate) bound: Bound,
}

#[derive(Debug)]
/// Parses `Clone` metas; the `enable_*` switches describe which parameters are allowed at the current position.
...
impl TypeAttributeBuilder {
    /// Parses one `Clone` meta into a `TypeAttribute`, rejecting parameters that are not enabled here.
    ...
    /// Scans the `#[educe(...)]` attributes of an item (typically an enum variant) and parses its `Clone` meta if present.
