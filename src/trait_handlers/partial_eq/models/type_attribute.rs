...
impl TypeAttributeBound {
    pub fn into_punctuated_where_predicates_by_generic_parameters(self, params: &Punctuated<GenericParam, Comma>) -> Punctuated<WherePredicate, Comma> {
        match self {
            ...
            TypeAttributeBound::Auto => create_where_predicates_from_generic_parameters(params, &syn::parse(quote!(core::cmp::PartialEq).into()).unwrap()),
...
impl TypeAttributeBuilder {
    pub fn from_partial_eq_meta(&self, meta: &Meta) -> TypeAttribute {
        ...
        let correct_usage_for_bound = {
            let usage = vec![stringify!(#[educe(PartialEq(bound))]), stringify!(#[educe(PartialEq(bound = "where_predicates"))]), stringify!(#[educe(PartialEq(bound("where_predicates")))])];
