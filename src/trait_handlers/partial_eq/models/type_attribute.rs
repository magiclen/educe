...
/// The parsed settings of a type-level (or variant-level) `PartialEq` attribute.
...
#[derive(Debug)]
/// Parses `PartialEq` metas; the `enable_*` switches describe which parameters are allowed at the current position.
...
impl TypeAttributeBuilder {
    ...
    /// Scans the `#[educe(...)]` attributes of an item (typically an enum variant) and parses its `PartialEq` meta if present.
