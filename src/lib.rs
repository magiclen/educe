            ...
            if let Meta::List(list) = &attr.meta {
                ...
                for meta in result {
                    ...
                    if let Some(v_meta) = trait_meta_map.get_mut(&t) {
                        // `Into` is the exception: it may appear once per target type.
...
/// Implements the traits listed in the `#[educe(...)]` attributes of the item.
///
/// See the [crate-level documentation](crate) for the traits this derive supports and the attributes each of them accepts.
